<template>
	<view class="container">
		...
		<view class="goods-section">
			...
			<!-- 商品列表 -->
			<block  v-for="(item, index) in itemShowList" :key="index" @click="showDetail(item)">
...
<script>
	...
	export default {
		...
		onLoad(option){
			...
			// this.orderData.paidStatus = Boolean(JSON.parse(option.paidStatus));
			// 
			this.loadData();
			this.calcTotal();  //计算总价
		},
		...
		methods: {			
			...
			deleteCartItem(index){
				...
				// this.itemShowList.splice(index, 1);
				this.calcTotal();
			},
			...
			quantityChange(data){
				...
				this.cartItems[data.index].quantity = data.number;
				this.calcTotal();
			},
			switchchange(e) {    
			...
			//
			loadData: function(){
				this.itemShowList = this.cartItems;
			},
			//显示优惠券面板
			toggleMask(type){
